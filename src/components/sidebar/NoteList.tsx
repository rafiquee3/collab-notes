...
export function NoteList({ selectedWorkspaceId, activeNoteId, onNoteSelect }: NoteListProps) {
  ...
  const handleCreateNote = (e: React.FormEvent) => {
    ...
    createNote.mutate({ workspaceId: selectedWorkspaceId, title: newNoteTitle.trim() });
  };
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-accent text-xs tracking-widest uppercase">Notes</h3>
        <button onClick={() => setIsCreating(true)} className="text-accent hover:text-foreground transition-colors" title="New Note">
            ...
            <li key={note.id} className="group relative">
              {editingNoteId === note.id ? (
                ...
                <div className="relative">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); startEditing(note.id, note.title); }} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 text-accent hover:text-foreground p-1 transition-all" title="Rename">
